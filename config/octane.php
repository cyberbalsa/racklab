...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | This value controls the maximum number of requests each Octane worker
    | will handle before being recycled. Recycling workers after a fixed
    | number of requests prevents long-lived static / singleton state from
    | accumulating across tenant boundaries. Set via OCTANE_MAX_REQUESTS in
    | .env — default 500 per the state-leak guard comment at the top of this
    | file.
    |
    */
