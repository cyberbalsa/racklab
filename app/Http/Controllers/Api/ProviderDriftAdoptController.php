...
final class ProviderDriftAdoptController extends Controller
{
    ...
    public function __invoke(
        ...
        if (! $tokenAbilities->allows($request, self::PERMISSION) || ! $decision->allowed) {
            $this->auditDenied($auditEvents, $user, $context, $drift);

            throw new AuthorizationException('You are not allowed to adopt provider drift.');
        }

        return response()->json([
            'data' => ProviderDriftPayload::make($resolver->adopt($drift, $user)),
        ]);
    }
