...
- The first GitHub image runs exposed two clean-checkout/production-image issues: the Containerfile initially copied `vite.config.ts` instead of the repo's `vite.config.js`, and Laravel production package discovery loaded `config/scribe.php` without the dev-only Scribe package installed. The Containerfile now copies the correct Vite config, and the Scribe config now uses literal strategy names/tuples so `--no-dev` images can load Laravel config while docs generation still works when Scribe is installed.
- Focused coverage now includes a no-autoloader Scribe config regression test that reproduces the `composer install --no-dev` package-discovery failure mode. Current focused gate: `vendor/bin/pest tests/Integration/ScribeConfigurationTest.php tests/Integration/OpenApiSchemaTest.php tests/Integration/BuildImagesWorkflowTest.php` passes with 11 tests / 298 assertions. `composer pint:test`, `composer larastan`, `composer rector:dry`, `git diff --check`, and `composer test` pass with 249 tests / 1706 assertions; the same two integration tests are skipped in the default SQLite profile. The image workflow is under active GitHub verification because this host has no Docker daemon and the local Podman runtime is not currently cgroup/subuid usable for builds.
