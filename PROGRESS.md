...
1. **`build-images-github`** — push/watch the next `.github/workflows/build-images.yml` run after the `BROADCAST_CONNECTION=null` image-build fix and fix any image-build, SBOM, or license-gate issues it surfaces.
...
The next concrete step is watching/fixing the next image workflow run after the broadcast-build fix, followed by external verification on a real Quadlet/systemd Baseline host or self-hosted runner.
