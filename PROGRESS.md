...
1. **`build-images-github`** — push/watch the next `.github/workflows/build-images.yml` run after the license-policy allowlist fix and fix any image-build, SBOM, or publish issues it surfaces.
...
The next concrete step is watching/fixing the next image workflow run after the license-policy allowlist fix, followed by external verification on a real Quadlet/systemd Baseline host or self-hosted runner.
