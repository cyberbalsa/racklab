...
- Current full gate: `composer validate --strict --no-check-publish`, `composer pint:test`, `composer larastan`, `composer rector:dry`, `composer security:racklab`, `composer openapi:check`, `composer audit`, `composer security:semgrep`, `composer pest:snapshots`, `composer i18n:missing`, `npm audit --audit-level=high`, `npm run build`, `git diff --check`, and `composer test` pass with 238 tests / 1590 assertions; the same two integration tests are skipped in the default SQLite profile. The real host-Podman runtime integration also passes with 1 test / 19 assertions.

## Next

Remaining MVP hardening after the deployment-lifecycle, script-approval, cloud-init, Proxmox, fake-runtime, audit-snapshot, OpenAPI-drift/body-detail/response-defaults/summaries, Semgrep/RackLab security checks, Podman-harness, dashboard Dusk, Baseline health/readiness, Baseline backup/restore, local worker-restart ops-smoke, host-Podman PostgreSQL/Redis release-smoke, Reverb live-broadcast, first M6 quota-reservation, first M6 placement-scheduler/anti-affinity primitive, M6 quota-admin/lease-policy, M5a network-offering/reachability/validation/admin-UI, M5b managed networking, M5b provider-drift repair/adoption, and host Podman runtime verification increments:

1. **`baseline-worker-host-soak`** — run the real systemd/worker version of the M2.5 drain/soak path on a Baseline host or self-hosted runner: restart `racklab-provider-worker@1` during active fake-provider work and run the 4-hour soak. The local host-Podman release smoke now covers PostgreSQL/Redis, Redis-backed ops-smoke backups, and restore of running fake-provider deployments; the remaining gap is the literal Quadlet/systemd worker drain. Horizon remains a dependency gap: current Packagist metadata for `laravel/horizon` v5.47.1 / 6.x-dev still allows Illuminate only through 12.x, so adding Horizon to this Laravel 13 repo is blocked until upstream compatibility changes or RackLab carries a fork.
2. **`release-smoke-github`** — execute the manual PostgreSQL 16 + Redis 7 browser release-smoke workflow in GitHub Actions after this MVP worktree is committed/pushed; `gh workflow list --repo cyberbalsa/racklab --all` currently cannot see the untracked local workflow files because they are not on the remote default branch yet.
3. **`ci-gates`** — monitor for a Symfony 8-compatible upstream release of `enlightn/security-checker` or an equivalent Laravel-specific scanner, and add any final custom Larastan behavior gaps that emerge from the MVP code surface.
4. **`packaging-release`** — cut the MVP release notes from `PROGRESS.md` after Baseline worker-host soak and PostgreSQL + Redis release-smoke pass are green.

The next concrete step is external verification: the Baseline worker host soak and the manual PostgreSQL 16 + Redis 7 release-smoke workflow after the local MVP worktree is on a branch GitHub Actions can run.
