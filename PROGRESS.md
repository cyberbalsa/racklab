...
2. **`podman-runtime-github`** — the workflow exists but requires a self-hosted runner labeled `linux`, `podman`, and `cgroup-delegated`; the attempted run was cancelled after remaining queued. Local host-Podman runtime verification is green.
...
4. **`packaging-release`** — cut the MVP release notes from `PROGRESS.md` after Baseline worker-host soak is green.

The next concrete step is external verification: the Baseline worker host soak on a real Quadlet/systemd Baseline host or self-hosted runner.
