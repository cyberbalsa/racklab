...
1. **`container-image-build-pipeline`** — add the M0.5 Containerfile/build workflow so the Baseline Quadlets have real `ghcr.io/cyberbalsa/racklab/*` images to pull.
...
5. **`packaging-release`** — cut the MVP release notes from `PROGRESS.md` after the image pipeline and Baseline worker-host soak are green.

The next concrete step is the container image build pipeline, followed by external verification on a real Quadlet/systemd Baseline host or self-hosted runner.
