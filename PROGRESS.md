...
- Current focused gate: `vendor/bin/pest tests/Integration/BuildImagesWorkflowTest.php` passes with 3 tests / 36 assertions. `composer pint:test`, `composer larastan`, `composer rector:dry`, `git diff --check`, and `composer test` pass with 248 tests / 1705 assertions; the same two integration tests are skipped in the default SQLite profile. The image workflow itself still needs its first GitHub run on the next push because this host has no Docker daemon and the local Podman runtime is not currently cgroup/subuid usable for builds.
