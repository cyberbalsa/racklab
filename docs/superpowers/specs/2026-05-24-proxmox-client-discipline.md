...
- **Dispatch after commit.** Horizon job dispatch is deferred until after the DB transaction commits by calling `->afterCommit()` on the dispatched job (the `$afterCommit` property on the queued job class). This is not `ShouldBroadcastAfterCommit`, which applies to broadcast events; for Horizon jobs the `afterCommit()` chained call or the `$afterCommit = true` property is the correct mechanism.
