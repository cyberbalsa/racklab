...
- Live-watch SSH output is relayed through the Horizon worker process via `phpseclib` to the browser's Reverb WebSocket. The narrow Track A JWT scoped to `(tenant, deployment_resource, op_set, expiry)` is the credential the worker validates before opening the SSH connection. The `ProviderConsoleProxy` unix-socket service is specific to Proxmox console operations (noVNC / xterm Proxmox terminal); SSH traffic does not traverse it.
