...
- **Scale profile** — multi-host, HashiCorp Nomad with the Podman driver. NATS queue-depth-driven worker autoscaling. The path when a deployment outgrows one host.
...
- (Scale only) `traefik`, `lego` cert agent, `nomad-server`, `nomad-client`, `prometheus`, `prometheus-nats-exporter`, `nomad-autoscaler`
...
- Autoscaling on NATS queue depth via Nomad Autoscaler + Prometheus + `nats-io/prometheus-nats-exporter`, scaling **one pool first** (default: `provider-worker`) in v1 and expanding per-pool as policies are tuned.
- PostgreSQL, NATS, and the Nomad agent itself remain Quadlets.
...
- Backup/restore documentation for PostgreSQL, NATS JetStream, artifact storage, plugin config, and ACME state (Traefik `acme.json` and `lego`-managed PEMs).
