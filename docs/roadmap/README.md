...
| M1 | [M01-auth-identity.md](M01-auth-identity.md) | Fortify + Socialite (OIDC/SAML via community providers), users/courses/projects/memberships, ProjectSSHKey, first-login personal Project creation, the share-link primitive, login UI, baseline branding (logo + theme variables). | 2–3 weeks |
| M2 | [M02-deployment-lifecycle.md](M02-deployment-lifecycle.md) | Catalog as Stack store, project-local Stacks, Default Stack, Stack Package export/import skeleton, deployment state machine, Reverb broadcast event stream with `Last-Event-ID` replay, Livewire dashboard, **fake provider** plugin proving the end-to-end loop. | 3–4 weeks |
...
| M3 | [M03-proxmox-provider.md](M03-proxmox-provider.md) | `ProxmoxClient` typed facade per spec, codegen-from-schema PHP Proxmox client + Guzzle transport integration with the task-polling discipline, Proxmox provider plugin, real clone/snapshot/power. | 3–4 weeks |
