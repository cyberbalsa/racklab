...
| # | File | Scope | Estimate |
|---|---|---|---|
| M0 | [M00-foundations.md](M00-foundations.md) | Repo skeleton, Django + uv, code CI gates, plugin framework, universal `Job` + `Artifact` models, RBAC primitives, audit subsystem, secret backend, i18n scaffolding. | 4–6 weeks |
| M0.5 | [M00.5-packaging-runtime-install.md](M00.5-packaging-runtime-install.md) | Container image build, Baseline install script, persistent plugin wheelhouse/config, SBOM/license scanning, runtime dependency audit, artifact/secret storage bootstrap. | 2–3 weeks |
| M1 | [M01-auth-identity.md](M01-auth-identity.md) | `django-allauth`, users/courses/projects/memberships, the share-link primitive, login UI, baseline branding (logo + theme variables). | 2–3 weeks |
| M2 | [M02-deployment-lifecycle.md](M02-deployment-lifecycle.md) | Catalog + catalog-versioning, deployment state machine, SSE event stream with `Last-Event-ID` replay, Mantine React-island dashboard, **fake provider** plugin proving the end-to-end loop. | 3–4 weeks |
...
| M3 | [M03-proxmox-provider.md](M03-proxmox-provider.md) | `ProxmoxClient` typed facade per spec, proxmoxer integration with the task-polling discipline, Proxmox provider plugin, real clone/snapshot/power. | 3–4 weeks |
...
| M7b | [M07b-script-sandbox.md](M07b-script-sandbox.md) | nsjail advanced-code runner, Ansible Runner integration, openQA-style console automation, approval workflow, runner artifacts. | 3–4 weeks |
...
| M9 | [M09-ssh-plugin.md](M09-ssh-plugin.md) | `racklab-console-ssh` per PRD §23 (Channels + asyncssh, host-key capture via cloud-init, redacted asciinema recording with abort-on-failure). | 3–4 weeks |
| M10a | [M10a-ui-component-library.md](M10a-ui-component-library.md) | Mantine + Radix-gap component library coverage for every PRD §15 Key Screen; React islands wired into Django pages; Storybook coverage for the component palette; LinguiJS catalogs for en-US complete. | 3–4 weeks |
...
| M11a | [M11a-tls-backend.md](M11a-tls-backend.md) | Traefik Baseline integration, self-signed bootstrap certs, dynamic config generation, TLS issuance backends, force-renew CLI, audit events. | 1–2 weeks |
...
| M12 | [M12-scale-profile.md](M12-scale-profile.md) | Nomad + Podman driver Scale profile per the orchestration spec, NATS-queue-depth autoscaling via Prometheus + the NATS exporter + Nomad Autoscaler, `lego` cert agent for Scale ACME, multi-host runbooks. | 4–6 weeks |
| M13a | [M13a-ha-data-tier.md](M13a-ha-data-tier.md) | HA Postgres, NATS failure drills, stateful-tier failover runbooks, and data-tier operational tests. | 2–3 weeks |
...
| M13c | [M13c-backup-restore-upgrade.md](M13c-backup-restore-upgrade.md) | Full backup/restore across Postgres, NATS, artifacts, plugin config, secrets, TLS state, plus Baseline/Scale upgrade drills. | 2–3 weeks |
...
5. **Deliverables**: the concrete artifacts (Django apps, plugins, models, admin pages, CLI commands, runbooks).
